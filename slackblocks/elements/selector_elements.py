...
class StaticSelect(Element):
    ...
    def _resolve(self) -> Dict[str, Any]:
        ...
        if self.options:
            ...
            element["option_group"] = self.option_group
...
class CheckboxSelector(Element):
    def __init__(self,
                 ...
                 initial_option: Optional[List[OptionObject]] = None,
                 confirm: Optional[Confirm] = None):
        super().__init__(ElementType.MULTI_USERS_SELECT)

        self.action_id = action_id
        self.options = options
        self.initial_options = initial_option
        self.confirm = confirm
    ...
    def _resolve(self) -> Dict[str, Any]:
        ...
        if self.initial_options:
            element["initial_option"] = [option._resolve() for option in self.initial_options]
        if self.confirm:
            element["confirm"] = self.confirm
        return element
